...
func createUpdate(updateDirectoryPath, distributionPath string) {
	...
	err = populateUpdateDirectory(diff)
...
func getDiff(updateLocationMap, distributionLocationMap *FileLocationInfo, inspectRootOnly bool) (*Diff, error) {
	diff := Diff{
...
func populateUpdateDirectory(diff *Diff) error {
	for filename, locationData := range diff.files {
		...
		case 0:
			err := handleNoMatch(filename, &locationData)
			util.HandleError(err)
		case 1:
...
//ok
func handleNoMatch(filename string, locationData *LocationData) error {
...
func generateLocationTable(filename string, locationsInDistribution map[string]bool) (*tablewriter.Table, map[string]string) {
	...
	for filepath, isDir := range locationsInDistribution {
		logger.Debug("[TABLE] filepath:", filepath, "; isDir:", isDir)
		indexMap[strconv.Itoa(index)] = filepath
		relativePath := "CARBON_HOME" + strings.TrimPrefix(filepath, distributionRoot)
